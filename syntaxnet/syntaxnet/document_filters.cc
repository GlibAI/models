...
class DocumentSource : public OpKernel {
 public:
  explicit DocumentSource(OpKernelConstruction *context) : OpKernel(context) {
    ...
    OP_REQUIRES_OK(context, context->GetAttr("documents_from_input", &documents_from_input_));
  ...
  void Compute(OpKernelContext *context) override {
    ...
    if (documents_from_input_) {
        ...
        std::unique_ptr<std::vector<std::string>> strings(new std::vector<std::string>);
...
//            LOG(INFO) << "text: " << input_vec(i);
        ...
        vec_reader.reset(new TextReader(*task_context_.GetInput(corpus_name_), &task_context_, std::move(strings)));
    }

    TextReader *reader = (documents_from_input_) ? vec_reader.get() : corpus_.get();
